...
#csvfile_name = f"fisie_state_data_ar_0-05_0.csv"
...
csvfile_name = f"fisie_merged_ar_{ratio}avg_pr.csv"
df = pd.read_csv(csvfile_name)

plt.figure(figsize=plt_figsize)
for i, s in enumerate(strats):
    df_strat = df[df['strategy']==s]
    avg_pr = df_strat['avg_pr']
    time = df_strat['Time']
    plt.plot(time, avg_pr, color=colours[i], label=s, linestyle=dashes[i])
plt.xlabel(r"Audit Cycle $t$")
plt.ylabel(r"Avg. Profit $G_f(t)$")
plt.legend(title="Legend — Strategies", title_fontproperties={'weight': 'bold', 'size': 'small'})
plt.title("Pass Rate over Time by Strategy")
plt.tight_layout()
#plt.yscale('log')
plt.show()
