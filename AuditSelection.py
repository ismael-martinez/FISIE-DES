...
def audit_selection(fog_nodes, oracle=True): #if false, IoT
    ...
    sum_rep = sum([math.exp(theta*f.reputation/T) for f in active_nodes])
    # Build distribution
    selection_dist = [0.0]*len(active_nodes)
    selection_dist[0] = math.exp(theta*active_nodes[0].reputation/T)/sum_rep
    for i in range(1, len(active_nodes)):
        selection_dist[i] = (math.exp(theta*active_nodes[i].reputation/T))/sum_rep + selection_dist[i-1]
